...
class BruteForce():
    """BruteForce approach who try all possible subsets with all different fractions"""
    ...
    def get_sorted_shares_list(csv_file):
        ...
        dataset = BruteForce.return_shares_list(csv_file)
    ...
    def main(shares_list):
        ...
        print(shares_list)
        print(f"Total cost: {BruteForce.return_price(shares_list)}")
        print(f"Total return: {BruteForce.return_profit(shares_list)}")


if __name__ == "__main__":
    dataset1 = BruteForce.get_sorted_shares_list("csv/dataset1_Python+P7.csv")
    dataset2 = BruteForce.get_sorted_shares_list("csv/dataset2_Python+P7.csv")
    share_wallet1 = []
    share_wallet2 = []
    wallet = 500

    shares_list1 = BruteForce.calculate_profitable_investment(
        dataset1, wallet, share_wallet1)
    shares_list2 = BruteForce.calculate_profitable_investment(
        dataset2, wallet, share_wallet2)
    BruteForce.main(shares_list1)
    BruteForce.main(shares_list2)
